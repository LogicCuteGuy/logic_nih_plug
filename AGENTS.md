...
This is a Rust audio-plugin framework (`nih_plug`) with many sub-crates and ~30 example/real plugins. Keep the codebase intact and reach for the smallest correct change.

> **Prefer linking to existing docs over restating them.** This file is the table of contents and the *nih_plug-specific* gotchas only.
...
- [QUICK_START.md](QUICK_START.md) — 5-minute intro to the JUCE-ported modules (`nih_plug_dsp`, `nih_plug_audio_formats`, `nih_plug_gui`, `nih_plug_animation`).
- `nih_plug::prelude` — every plugin starts with `use nih_plug::prelude::*;`. Read [src/prelude.rs](src/prelude.rs) when in doubt about what is re-exported.
...
> **Never** run `cargo test --all-features` — `nih_plug_iced` has mutually exclusive features. The `--features "simd,standalone,zstd"` set above is what CI uses.

The `xtask` alias is defined in [.cargo/config.toml](.cargo/config.toml); it runs the `xtask` shim binary in release mode so `serde(-derive)` is only built once. The shim itself lives in [xtask/src/main.rs](xtask/src/main.rs) and just calls `nih_plug_xtask::main()`. CI pins `dtolnay/rust-toolchain@nightly` because `simd` needs nightly `std::simd`.
...
| Path | Role |
|---|---|
| [src/](src/) | The `nih_plug` core library: `Plugin` trait, `Buffer`, `ProcessContext`, `Params` system, per-API wrappers, `debug`/`util`/`formatters`. |
| `nih_plug_derive/` | `#[derive(Params)]` proc macro. |
| `nih_plug_xtask/` + `xtask/` | Bundling library + in-repo shim. Vendored into downstream plugin repos — see its README. |
| `cargo_nih_plug/` | `cargo nih-plug` subcommand (same engine as `cargo xtask`). |
| `nih_plug_egui` / `_iced` / `_vizia` | GUI backends. |
| `nih_plug_gui` | BYO-GUI helpers **and** a JUCE-style `Component`/`Button`/FlexBox port. Features: `components`, `layout`, `graphics`, `text`, `softbuffer-editor`, `gl-editor`, `full`. |
| `nih_plug_dsp` | Filters, oscillators, convolution, envelopes, smoothing, processors, analysis. Features: `filters`, `oscillators` (default), plus optional ones; `full` turns them all on. |
| `nih_plug_audio_formats` | WAV/AIFF (+ optional FLAC/OGG) encoding. |
| `nih_plug_graphics` | 2D primitives (+ optional images/text). |
| `nih_plug_animation` | Easing + chaining. |
...
8. **Use `nih_log!`/`nih_dbg!`/`assert_*` from `nih_plug::debug`** instead of `println!`/`eprintln!`/`dbg!`/`assert!`.
...
- **For tests, mirror the sub-crate's existing layout** (e.g. `nih_plug_dsp/tests/`). Use `proptest` for property tests; the framework already standardises on `criterion = "0.5"` for benches.
...
- **Want to add a parameter smoothing style / new param type?** Look at [src/params/](src/params/) and the derive crate `nih_plug_derive/`.
